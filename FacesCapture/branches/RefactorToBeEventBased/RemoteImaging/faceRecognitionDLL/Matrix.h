...
******/
	...
	//【注意】这个接口函数有问题，不能使用！！！
	//valarray的构造函数会将原来的内存拷贝到新的一段空间，而不是指向同一空间。
/******
 矩阵类matrix的构造函数二
     对私有变量m_stRow和m_stCol分别赋初值stRow, stCol。
...
//calc the norm of vector
//Returns sum(abs(A).^2)^(1/2)
template <class _Ty>
long double VectorNorm(matrix<_Ty>& mIn);

//mean on matrix
template <class _Ty>
bool MatrixMean(matrix<_Ty>& mIn, int Dim,matrix<_Ty>& mOut);
